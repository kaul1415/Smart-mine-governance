...
// Toggle for the whole app via VITE_USE_MOCKS environment variable. Defaults to false if set to 'false'.
...
async function request(path, { method = 'GET', body, headers, signal } = {}) {
  let token = localStorage.getItem('minegov_auth_token');
  if (
    !token ||
    typeof token !== 'string' ||
    token === 'undefined' ||
    ...
    token = null;
    if (localStorage.getItem('minegov_auth_token')) {
  ...
  if (!response.ok) {
    // If backend reports invalid/malformed token or 401, purge corrupted token to prevent repeating errors
    if (response.status === 401) {
      localStorage.removeItem('minegov_auth_token');
      localStorage.removeItem('minegov_auth_user');
      window.dispatchEvent(new Event('minegov_auth_cleared'));
    }
    throw new ApiError(payload?.message || response.statusText, response.status, payload);
  }

  // Auto-unwrap backend envelope { success: true, data: ... } for seamless frontend consumption
